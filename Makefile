...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting research-assistant-agent in development mode..."
	@uv run watchfiles "python -m research_assistant_agent" research_assistant_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t research_assistant_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t research_assistant_agent:latest .
...
docker-run: ## Run Docker container with .env file
	...
	@docker run -d \
		...
		research_assistant_agent:latest
...
docker-stop: ## Stop Docker container
	...
	@docker stop research_assistant_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm research_assistant_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f research_assistant_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it research_assistant_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi research_assistant_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag research_assistant_agent:latest estherd004/research_assistant_agent:latest
	@docker push estherd004/research_assistant_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep research_assistant_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep research_assistant_agent || echo "No images found"
	@echo "\nAgent Health:"
